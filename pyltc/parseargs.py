...
def handle_ingress_arg(ingress_arg, verbose_arg, parser):
    ...
    if ingress_arg != 'setup':
        parser.error("ifb device not found: {!r}. Leave --ingress switch empty to set up a new one.".format(ingress_arg))
    ...
    CommandLine('ip link set dev {} up'.format(ifbdev)).execute(verbose=verbose_arg)
    return ifbdev
...
def parse_args(argv, old_args_dict={}):
